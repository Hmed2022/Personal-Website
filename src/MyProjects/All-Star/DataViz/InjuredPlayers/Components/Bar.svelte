<script context="module">
	...
	export function getStackExtents(data, groupBy, stackBy, offset) {
		const pivotData = pivot(data, groupBy, stackBy, items => sum(items, d => d.percentage_of_votes));
...
<script>
	...
	$: pivotData = pivot($data, groupBy, stackBy, items => sum(items, d => d.percentage_of_votes));
...
<g class="column-group">
  {#each chartData as item, i (`${item.data.year}-${item.key}`)}
    ...
    />
		...
		<!-- <text  x={getDimensions(item).x} y={getDimensions(item).y + getDimensions(item).height / 2} class="label">
      {item.key}
    </text> -->
	{/each}
</g>