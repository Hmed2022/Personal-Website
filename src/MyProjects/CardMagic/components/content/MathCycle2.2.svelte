<script>
  ...
  // bounds of the cards matching BOTH cycle-1 and cycle-2 piles (the narrowed
  // set of candidates), for the small callout brace in the positions view
  function bothBounds(){
    if (!browser || !board) return null;
    const b = board.getBoundingClientRect();
    ...
    for (const card of arr) {
      if (matchClass(card) !== "both") continue;
      const el = elMap.get(getId(card));
      if (!el) continue;
      const r = el.getBoundingClientRect();
      y1 = Math.min(y1, r.top - b.top);
  ...
  let bothBox = null;
  $: layoutVersion, bothBox = bothBounds();
...
<main class="body">
  {#if $statuscard}
    {#if $annotations}
      <div class="descPos" in:fade={{ duration: 300 }} out:fade={{ duration: 200 }}>
        <div class="desc">
    ...
    <div class="stage" bind:this={board}>
      ...
      <!-- Positions overlay -->
      {#if $positions}
        <div class="overlay-fade" in:fade={{ duration: 300 }} out:fade={{ duration: 200 }}>
          ...
          <div class="brace-layer" aria-hidden="true">
            ...
            {#if bothBox}
              {#key layoutVersion}
                <CurlyBraceCoords side="right" x={posX - 60} y1={bothBox.y1} y2={bothBox.y2}
                                  stroke="#56993D" strokeWidth={4} w={20} q={0.6}/>
              {/key}
            {/if}
          </div>
        </div>
    ...
    {#if $positions}
      <div class="descPos1" in:fade={{ duration: 300 }} out:fade={{ duration: 200 }}>
        <p>
          After cycle 2 we reduced the potential selected cards from 9 to 3.
          These potential matches are now in the middle deck positioned between
          the 13th and 15th positions. Since these cards are consecutive, after
          another cycle, each card will be the middle card in one of the three
          divided piles.
        </p>
      </div>
    {/if}
    ...
    {#if $annotations}
      <div class="note-cycle" in:fade={{ duration: 300 }} out:fade={{ duration: 200 }}>
